...
class OrderController extends ControllerAuth
{
	 ...
	 * @return [type] [description]
	 */
	public function stripeReadyToPayAction()
	{
        ...
        $strip_payment_method_id = $this->post['stripe_payment_method_id'];
		...
		if ($payment_method == 'stripe') {
            ...
            $stripe_api_key = 'sk_test_StVw5udtQT2bYewQC1vfVdH800N9zMnqk0';
    ...
    public function readyToPayAction()
	{
        ...
        fputs($log, $payment_method . PHP_EOL);
        $payment_data = [];
		...
		if ($payment_method == 'stripe') {
            ...
            //2019-10-07 应用新版stripe支付方式，注释保留旧版支付方式 
		...
		try {
			if ($payment_method == 'stripe') {
                ...
                /* $charges = \Stripe\Charge::all([
                    'payment_intent' => $strip_payment_intent_id,
                ]); */
	...
	public function payAction()
	{
		...
		try {
            $paid_result = false;
            $pay_transaction_id = null;

			if ($pay_token == 'test') {
                
                $paid_result = true;
                $pay_transaction_id = 'test';
            } 
            elseif ($payment_method == 'stripe') {
				...
				if($charge->paid){
                    ...
                    $paid_result = true;
                    $pay_transaction_id = isset($charge->id) ? $charge->id : ''; 
                ...
                //新版stripe支付方式
                $intent = \Stripe\PaymentIntent::retrieve($pay_token);
                $charges = $intent->charges->data;
                $charge = $charges[0];
                fputs($log, var_export($charge,true) . PHP_EOL);
